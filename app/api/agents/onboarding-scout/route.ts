...
async function runScout(potterId: string, websiteUrl: string) {
...
If no clear individual products are found, return an empty array [].`;
  ...
  console.log(`[onboarding-scout] Done for potter ${potterId}.`);
}
...
export async function POST(req: Request) {
  ...
  // Parse the Supabase DB webhook payload
  ...
  if (!websiteUrl) {
    // No website provided — nothing to crawl, that's fine
    console.log(`[onboarding-scout] Potter ${potterId} has no website, skipping.`);
    return Response.json({ received: true, skipped: true });
  }

  // Return 200 immediately and process in the background
  waitUntil(runScout(potterId, websiteUrl));
